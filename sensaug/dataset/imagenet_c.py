...
class DistortImageFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        save_path = (
            "/share/data/vision-greg/DistortedImageNet/JPEG/"
...
def clipped_zoom(img, zoom_factor):
    ...
    side = (w - cw) // 2
    img = scizoom(
        img[top : top + ch, side : side + cw], (zoom_factor, zoom_factor, 1), order=1
    )
...
def fog(x, severity=1):
    ...
    max_val = x.max()
    mapsize = 2 ** np.ceil(
        np.log2(max(width, height))
    )  # mapsize needs to be a power of 2
    temp = plasma_fractal(mapsize=int(mapsize), wibbledecay=c[1])
    fractal = c[0] * temp[:height, :width][..., np.newaxis]
    x += fractal
...
def frost(x, severity=1):
    ...
    frost = cv2.imread(os.path.join("sensaug", "dataset", "frost", filename))

    # increase image size until it covers the image dims entirely
    while (frost.shape[0] - height) <= 0 or (frost.shape[1] - width) <= 0:
        frost = cv2.resize(
            frost,
            (frost.shape[1] * 2, frost.shape[0] * 2),
            interpolation=cv2.INTER_AREA,
        )

    # randomly crop and convert to rgb
    x_start, y_start = (
        np.random.randint(0, frost.shape[0] - height),
        np.random.randint(0, frost.shape[1] - width),
    )
...
def driving_perturbations(src_folder, output, img_h, img_w):
    import collections
    ...
    for method_name in d.keys():
        save_distorted(src_folder, output, d[method_name])
