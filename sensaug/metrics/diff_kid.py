...
class DiffKernelInceptionDistance(Metric):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        feature_extractor_resnet50 = torch.hub.load(
            "NVIDIA/DeepLearningExamples:torchhub", "nvidia_resnet50", pretrained=True
        )
        # feature_extractor_resnet50 = torch.hub.load('pytorch/vision:v0.10.0', 'inception_v3', pretrained=True)
        modules = list(feature_extractor_resnet50.children())[:-1]
        feature_extractor_resnet50 = torch.nn.Sequential(*modules)

        for p in feature_extractor_resnet50.parameters():
            p.requires_grad = False

        self.inception = feature_extractor_resnet50
        self.inception.eval()
    ...
    def update(self, imgs: Tensor, real: bool) -> None:
        ...
        self._update(imgs, real=real)

    def _update(self, imgs: Tensor, real: bool):
        """Update the state with extracted features."""
