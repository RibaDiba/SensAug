...
def calculate_fid_given_list_files(list_files, batch_size, cuda, dims):
    ...
    m1, s1 = calculate_activation_statistics(one_list0, model, batch_size, dims, cuda)
    m2, s2 = calculate_activation_statistics(one_list1, model, batch_size, dims, cuda)
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)

    return fid_value


def calculate_fid_given_list_files_csv(
    list_files_pair, data_folders_pair, batch_size, cuda, dims
):
    one_list0 = []
    ...
    for i in range(len(paths)):
        for j in range(len(paths)):
            fid_value = calculate_frechet_distance(
                m_array[i], s_array[i], m_array[j], s_array[j]
            )
