...
def parse_args():
    ...
    parser.add_argument("-o", "--out_dir", help="output path", default="./data/acdc")
    parser.add_argument(
        "--split", choices=["fog", "night", "rain", "snow", "all"], default="night"
    )
...
def main():
    ...
    for subdir in [
        f"gt{suffix}/test",
        f"rgb_anno{suffix}/test",
        f"gt{suffix}/train",
        f"rgb_anno{suffix}/train",
    ]:
    ...
    for dirpath, _, filenames in os.walk(args.raw_data):
        for filename in filenames:
            ...
            if anno_str_test in full_path:
                if full_path.endswith("_rgb_anon.png"):
                    new_path = os.path.join(
                        args.out_dir, f"rgb_anno{suffix}/test", filename
                    )
            ...
            if anno_str_train in full_path:
                if full_path.endswith("_rgb_anon.png"):
                    new_path = os.path.join(
                        args.out_dir, f"rgb_anno{suffix}/train", filename
                    )
                    if full_path != new_path:
                        shutil.copy(full_path, new_path)
